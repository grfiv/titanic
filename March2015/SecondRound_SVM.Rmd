...
pct produced my best score prior to this: 0.80861  
...
file_name        = "submission_svm_2.csv"
...
for (i in 2:length(form.vars)) {
  ...
  matrix = table(actual    = raw_data$Survived[test.set], 
                 predicted = attributes(svm.pred)$probabilities[,1] >= threshold)
...
# # ... except my over-rides give better results and with
# #     high bias I'm not worried about overfitting
...
matrix = table(actual    = raw_data$Survived[test.set], 
  ...
  ROCRpred = prediction(attributes(svm.pred)$probabilities[,1], raw_data$Survived[test.set])
  ROCRperf = performance(ROCRpred, "tpr", "fpr")
  AUC = performance(ROCRpred,"auc")@y.values[[1]]

  comparison = data.frame(svm=c(round(model_accuracy,digits=4),
                                round(mse,digits=4),
...
print(comparison)
print(matrix)
plot_thresholds(attributes(svm.pred)$probabilities[,1],threshold)
```  
...
#                         replace=TRUE)             # bootstrap for n
  ...
  svm.fit.LC = svm(formla, data=raw_data[train.set.LC,],
                    kernel="radial",
                    scale=FALSE, probability=TRUE,decision.values=TRUE,
                    gamma=best.gamma,
                    cost=best.cost)
  ...
  svm.pred.train.LC = predict(svm.fit.LC, newdata=raw_data[train.set.LC,], 
                              decision.values=TRUE, probability=TRUE)

  svm.pred.test.LC = predict(svm.fit.LC, newdata=raw_data[test.set,], 
                             decision.values=TRUE, probability=TRUE)
  
  # find the MSE for train/test-set predictions
  mse.train.LC = mean((attributes(svm.pred.train.LC)$probabilities[,1] - 
                         raw_data[train.set.LC,]$Survived)^2)
  mse.test.LC  = mean((attributes(svm.pred.test.LC)$probabilities[,1]  - 
                         raw_data[test.set,]$Survived)^2)
...
```{r FP}
pred.vectors = data.frame(predicted = as.numeric(attributes(svm.pred)$probabilities[,1] >= threshold), 
...
if (submit_to_kaggle) {
  ...
  submission.df = data.frame(PassengerId=Xtest$PassengerId,
                             Survived=as.numeric(attributes(svm.pred)$probabilities[,1] <= threshold))
  #                                   --------------------------------
