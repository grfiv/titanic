...
file_name        = "submission_boost_2.csv"
...
------------
See the dataset *boostParameters.R* for exhaustive CV analysis. Note that it uses *formla = formula(Survived~.-PassengerId)* and it should be rerun after the formula is set here (a) to find new parameters and (b) to ensure that the new formula actually does perform better.
```{r boostparms}
boost.shrinkage = 0.0001
boost.ntrees    = 60000
boost.depth     = 1
```
...
formla = formula("Survived ~ -PassengerId + Pclass + Sex + Age + SibSp + Parch + 
     Embarked + title + FamilySize + I(as.numeric(Deck) * FamilySize) + 
     I(as.numeric(Pclass) * Age) + I(as.numeric(Pclass) * SibSp) + 
     I(as.numeric(Pclass) * as.numeric(Embarked)) + I(as.numeric(Pclass) * 
     FamilySize) + I(as.numeric(Pclass) * as.numeric(Side)) + 
     I(as.numeric(Sex) * SibSp) + I(as.numeric(Sex) * FamilySize) + 
...
if (run_forward_selection) {
  ...
  #                  + poly(I(as.numeric(Deck)*FamilySize),2)
  #                  + poly(I(as.numeric(Deck)*as.numeric(Side)),2)
  ...
  #                  + poly(I(as.numeric(Embarked)*as.numeric(Fare2)),2)
...
#                    + poly(Age,3)   
  ...
  for (i in 2:length(form.vars)) {
    ...
    boost.fit = gbm(formla, 
                    ...
                    interaction.depth = boost.depth,
                    shrinkage         = boost.shrinkage)
    
    boost.pred = predict(boost.fit, 
                         newdata=raw_data[test.set,],
                         n.trees=boost.ntrees,
                         type="response")
    
    matrix = table(actual=raw_data$Survived[test.set], 
                   predicted=boost.pred >= threshold)
    ...
    # if accuracy improves or (stays the same and MSE goes down), keep the feature
    if ( (model_accuracy > accuracy) || 
        ((model_accuracy == accuracy) && (mse < meansqe)) ) {
    #if (model_accuracy >= accuracy) {
    #if (model_accuracy > accuracy) {   # many fewer vars, acc not quite as good
      old.vars = new.vars
...
matrix = table(actual=raw_data$Survived[test.set], predicted=boost.pred >= threshold)
  ...
  comparison = data.frame(boost=c(round(model_accuracy,digits=4),
                                round(mse,digits=4),
...
print(comparison)
print(matrix)
plot_thresholds(boost.pred,threshold)
```  
...
#                         replace=TRUE)             # bootstrap for n
  ...
  boost.pred.train.LC = predict(boost.fit.LC,                    ###
                                newdata=raw_data[train.set.LC,], ###
                                n.trees=boost.ntrees,
                                type="response")
  
  boost.pred.test.LC = predict(boost.fit.LC,                     ###
                               newdata=raw_data[test.set,],
                               n.trees=boost.ntrees,
                               type="response")
  
  # find the MSE for train/test-set predictions
  mse.train.LC = mean((boost.pred.train.LC - raw_data[train.set.LC,]$Survived)^2)
  mse.test.LC  = mean((boost.pred.test.LC  - raw_data[test.set,]$Survived)^2)
...
```{r FP}
pred.vectors = data.frame(predicted = as.numeric(boost.pred>=threshold), 
...
if (submit_to_kaggle) {
  ...
  submission.df = data.frame(PassengerId=Xtest$PassengerId,
                             Survived=as.numeric(boost.pred >= threshold))
  #                                   --------------------------------
