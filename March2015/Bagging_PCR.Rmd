...
setwd("C:/Users/GRFisher4/Dropbox/kaggle/Classification Titanic/March2015")
...
file_name        = "submission_bagging_pcr.csv"
...
## Fit the model on the FULL TRAINING DATASET and Predict on Xtest   
...
if (submit_to_kaggle) {
    ...
    accumulator = data.frame(rep(0,nrow(Xtest)))
    
    for (i in 1:iterations) {
    
        rm(pcr.fit, best.comp, pcr.pred)
        
        #index = sample(1:nrow(raw_data),nrow(raw_data)/2,replace=FALSE)
        ...
        pcr.pred = predict (pcr.fit, 
                          newdata=Xtest,         #############
                          ncomp=best.comp-1, type="response")
        
        
        accumulator = accumulator + data.frame(pcr.pred)[,1]
    }
    
    accumulator = accumulator / iterations
    
    pcr.pred = accumulator
